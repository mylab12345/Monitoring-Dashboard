...
while [ $# -gt 0 ]; do
  case "$1" in
    --port)     PORT="$2"; shift 2 ;;
    --bind)     BIND="$2"; BIND_SET=1; shift 2 ;;
...
# Preserve old bind if user did not explicitly set --bind and old exists
if [ "$BIND_SET" -eq 0 ] && [ -n "$OLD_BIND" ]; then
  BIND="$OLD_BIND"
fi
...
if [ "$EXPOSED" -eq 1 ] && [ -n "$TOKEN" ]; then
  if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -q "Status: active"; then
    log "Opening port $PORT/tcp in ufw (token-protected)"
    ufw allow "$PORT/tcp" >/dev/null 2>&1 || true
  elif command -v firewall-cmd >/dev/null 2>&1 && firewall-cmd --state >/dev/null 2>&1; then
    log "Opening port $PORT/tcp in firewalld (token-protected)"
    firewall-cmd --permanent --add-port="$PORT/tcp" >/dev/null 2>&1 && firewall-cmd --reload >/dev/null 2>&1 || true
  fi
elif [ "$EXPOSED" -eq 1 ] && [ -z "$TOKEN" ]; then
  warn "Firewall NOT opened automatically because no token is set — protect the dashboard first with --token."
fi
