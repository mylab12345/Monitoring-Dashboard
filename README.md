...
- If `upgrade-packages` / **Fix Broken** reports `Read-only file system` or leaves a package `half-installed`, the dashboard can repair it itself: the Maintenance card offers **Repair service mount namespace** (and a failed fix prompts you automatically). That runs the whitelisted `monitoring-self-repair` helper via the service account: it patches the installed unit with `ReadWritePaths=/usr /etc /boot /efi`, runs `systemctl daemon-reload`, restarts `monitoring`, then retries your fix. If the dashboard itself cannot reach writable filesystems (host genuinely mounted read-only), fall back to the manual path: `sudo ./update.sh` from this checkout (or `sudo /opt/monitoring/update.sh --remote`), then `sudo systemctl daemon-reload && sudo systemctl restart monitoring`. Confirm the diagnosis with `monitoring check-privileges`, `GET /api/privileges` or `GET /api/self-repair`; retry **Fix Broken** after the package filesystem is writable.
